...
/*NOTE: for this project, I did the extra credit.  You can call a function called ls-X2*/
...
{  
  ...
  int go = 1, ampersand = 0, noclobber = 0;
  char **args; 
  ...
  while ( go )
  {
    ...
    args = redirection(args);                            /*Supports file redirection*/
    ...
    else if(args[0] == NULL){ }                         /*If Null*/
    else if(strcmp(args[0], " ") == 0){ }               /*Nothing is typed*/
    ...
    close(0); close(1); close(2);                 /*Reset streams to default*/
...
int builtin(char **args, struct pathelement *pathlist, char **envp)
{
  ...
  {
    if(args[1] == NULL)                 /*return to homedirectory*/
    {
      ...
      status = 2;
    else if(strcmp(args[1], "-") == 0)  /*return to previous directory*/
    {
  ...
  else if(strcmp(args[0], "ls-X") == 0 || strcmp(args[0], "ls-X2") == 0)      /*ls-X, ls-X2*/   /*EXTRA CREDIT!!!!*/
  {
    if(args[1] == NULL)   /*no arguments lists the current directory*/
    {    
      if(strcmp(args[0], "ls-X") == 0)
        ls_X(".", 0);
      else
        ls_X(".", 1);
    }
  ...
  else if(strcmp(args[0], "noclobber") == 0)                  /*noclobber*/
  {
    if(args[1] != NULL)  /*too many arguments*/
      status = 2;
    else
    {
      if(noclobber == 0)
        ...
        noclobber = 0;
       printf("Noclobber value set to: %d\n", noclobber);
    }
  }
  else
    return 0;
...
char *which(char *command, struct pathelement *pathlist)
{
  ...
  while(tmp->element != NULL)
  {
    ...
    if(access(path, F_OK) == 0 && access(path, X_OK) == 0)
    {
      ...
      if(!S_ISDIR(filestats.st_mode))   /*If the path isn't a directory*/ 
        return path;  
    }
    tmp = tmp ->next;  
  }
...
char **redirection(char **args)
{
  int count = 0, flag = 1;
  char **arglist = calloc(MAXARGS, sizeof(char*));
  char *redirector = calloc(1, sizeof(char*));
  struct stat filestats;
  stat("/dev/tty", &filestats);

  while(args[count] != NULL && flag)
  {
    if(count == 0)
      arglist[0] = args[0];
    else if(strcmp(args[count], ">") == 0 || strcmp(args[count], ">&") == 0 
                   || strcmp(args[count], ">>") == 0 || strcmp(args[count], ">>&") == 0
                   || strcmp(args[count], "<") == 0)   /*Check for directors*/
    {
      flag = 0;
      redirector = args[count];
    }
    else    
      arglist[count] = args[count];

    ++count;
  }

  if(!flag && !(args[count] != NULL))         /*File director found but no file name specified*/
  {
    fprintf(stderr, "No file specified.\n");
    arglist[0] = NULL;
  }
  else if(args[count + 1] != NULL)            /*Too many arguments*/
  {
  ...
  {
    if(strcmp(redirector, ">") == 0)                   /*Stdout to file*/
    {
      if(noclobber)
      {
        if(access(args[count], F_OK) == 0)
        ...
        {
          fd1 = open(args[count] , O_CREAT|O_WRONLY, filestats.st_mode);
          dup2(fd1, 1);
          close(fd1);
        }
      }
      ...
      {
        fd1 = open(args[count] , O_CREAT|O_WRONLY|O_TRUNC, filestats.st_mode);
        dup2(fd1, 1);
        close(fd1);
      }
    }
    else if(strcmp(redirector, ">&") == 0)             /*Stdout & Stderr to file*/
    {
      if(noclobber)
      {
        if(access(args[count], F_OK) == 0)
        ...
        {
          fd1 = open(args[count] , O_CREAT|O_WRONLY, filestats.st_mode);
          fd2 = open(args[count] , O_CREAT|O_WRONLY, filestats.st_mode);
          dup2(fd1, 1);
          dup2(fd2, 2);
          close(fd1);
          close(fd2);
      ...
      {
        if(access(args[count], F_OK) == 0) unlink(args[count]);
        fd1 = open(args[count] , O_CREAT|O_WRONLY|O_APPEND, filestats.st_mode);
        fd2 = open(args[count] , O_CREAT|O_WRONLY|O_APPEND, filestats.st_mode);
        dup2(fd1, 1);
        dup2(fd2, 2);
        close(fd1);
        close(fd2);
    ...
    else if(strcmp(redirector, ">>") == 0)             /*Append Stdout to file*/
    {
      if(noclobber)
      {
        if(access(args[count], F_OK) == 0)
        {
          fd1 = open(args[count] , O_WRONLY|O_APPEND, filestats.st_mode);
          dup2(fd1, 1);
          close(fd1);
        }
        else
    ...
    else if(strcmp(redirector, ">>&") == 0)            /*Append Stdout & Stderr to file*/
    {
      if(noclobber)
      {
        if(access(args[count], F_OK) == 0)
        {
          fd1 = open(args[count] , O_WRONLY|O_APPEND, filestats.st_mode);
          fd2 = open(args[count] , O_WRONLY|O_APPEND, filestats.st_mode);
          dup2(fd1, 1);
          dup2(fd2, 2);
          close(fd1);
          close(fd2);
    ...
    else if(strcmp(redirector, "<") == 0)             /*Stdin redirection*/
    {
      if(access(args[count], F_OK) == 0)
      {
        fd1 = open(args[count] , O_RDONLY, filestats.st_mode);
        dup2(fd1, 0);
        close(fd1);
      }
      else
