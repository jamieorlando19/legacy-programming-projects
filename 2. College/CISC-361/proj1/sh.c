...
{  
  ...
  while ( go )
  {
    ...
    else if(strcmp(args[0], "prompt") == 0)                   /*Prompt is typed*/
    {
      ...
      forkexec(which(args[0], pathlist), args, envp);
    else if((commandline[0] == '/') || (commandline[0] == '.'))   /*Absolute path*/
      abspath(commandline, args, envp);
...
void forkexec(char *path, char **argv, char **envp)
{
...
void abspath(char *commandline, char **arglist, char **envp)
{
